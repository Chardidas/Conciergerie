...
})().directive('customButtonWithPrint', function() {
    return {
      controllerAs: 'ctrl',
      controller: function PbButtonCtrl($scope, $http, $location, $log, $window, $sce) {
  ...
  function doRequest(method, url, params) {
    ...
    return $http(req)
      .success(function(data, status) {
        ...
        if ($scope.properties.print === true){
            printPdf(data);
        }
      })
    ...
    function printPdf (url){
        $http({
            ...
            $log.log('Sorry, something went wrong');
        });
    }
...
,
      template: '<div class="text-{{ properties.alignment }}">\n    <button\n        ng-class="\'btn btn-\' + properties.buttonStyle"\n        ng-click="ctrl.action()"\n        type="button"\n        ng-disabled="properties.disabled || ctrl.busy" ng-bind-html="ctrl.busy?\'Construction du pdf ...\':properties.label | uiTranslate"></button>\n</div>\n'
    };
  });
